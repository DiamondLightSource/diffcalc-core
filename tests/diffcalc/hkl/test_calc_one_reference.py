...
def test_fixed_chi_phi_a_eq_b(cubic: HklCalculation, case: Case):
    cubic.constraints = configure_constraints({"chi": 90, "phi": 0, "a_eq_b": True})
...
def test_with_one_ref_and_mu_0_phi_0(cubic: HklCalculation, case: Case):
    cubic.constraints = configure_constraints({"psi": 90, "mu": 0, "phi": 0})
...
def test_with_one_ref_and_eta_0_phi_0(cubic: HklCalculation, case: Case):
    cubic.constraints = configure_constraints({"psi": 0, "eta": 0, "phi": 0})
...
def test_i07_horizontal(cubic: HklCalculation, case: Case, places: int):
    configure_ub(cubic.ubcalc, u_matrix=I)
    cubic.constraints = configure_constraints({"chi": 0, "phi": 0, "a_eq_b": True})

    convert_position_to_hkl_and_hkl_to_position(cubic, case, places)


def test_i07_horizontal_fails_for_parallel_vectors(cubic: HklCalculation):
    configure_ub(cubic.ubcalc, u_matrix=I)
    cubic.constraints = configure_constraints({"chi": 0, "phi": 0, "a_eq_b": True})
...
def test_i16_vertical(cubic: HklCalculation, case: Case):
    configure_ub(cubic.ubcalc, u_matrix=I)
    cubic.constraints = configure_constraints({"chi": 90, "psi": 90, "phi": 0})
...
def test_i16_failed_hexagonal_experiment_with_small_variations(hklcalc: HklCalculation):
    ...
    hklcalc.constraints = configure_constraints(
        {"delta": 0, "alpha": 17.8776, "eta": 0}
    )
