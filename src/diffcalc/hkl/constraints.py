...
class Constraints:
    ...
    def __init__(
        ...
        self._delta = Constraint("delta", CATEGORY.DETECTOR, TYPE.VALUE)
        self._nu = Constraint("nu", CATEGORY.DETECTOR, TYPE.VALUE)
        self._qaz = Constraint("qaz", CATEGORY.DETECTOR, TYPE.VALUE)
        self._naz = Constraint("naz", CATEGORY.DETECTOR, TYPE.VALUE)

        self._a_eq_b = Constraint("a_eq_b", CATEGORY.REFERENCE, TYPE.VOID)
        self._alpha = Constraint("alpha", CATEGORY.REFERENCE, TYPE.VALUE)
        self._beta = Constraint("beta", CATEGORY.REFERENCE, TYPE.VALUE)
        self._psi = Constraint("psi", CATEGORY.REFERENCE, TYPE.VALUE)
        self._bin_eq_bout = Constraint("bin_eq_bout", CATEGORY.REFERENCE, TYPE.VOID)
        self._betain = Constraint("betain", CATEGORY.REFERENCE, TYPE.VALUE)
        self._betaout = Constraint("betaout", CATEGORY.REFERENCE, TYPE.VALUE)

        self._mu = Constraint("mu", CATEGORY.SAMPLE, TYPE.VALUE)
        self._eta = Constraint("eta", CATEGORY.SAMPLE, TYPE.VALUE)
        self._chi = Constraint("chi", CATEGORY.SAMPLE, TYPE.VALUE)
        self._phi = Constraint("phi", CATEGORY.SAMPLE, TYPE.VALUE)
        self._bisect = Constraint("bisect", CATEGORY.SAMPLE, TYPE.VOID)
        self._omega = Constraint("omega", CATEGORY.SAMPLE, TYPE.VALUE)

        self._all: Tuple[Constraint, ...] = (
            self._delta,
            self._nu,
            self._qaz,
            self._naz,
    ...
    def constrained(self, category: CATEGORY = None) -> Set[Constraint]:
        """Get all active constraints.

        Returns
        -------
        Set[Constraint]
            Set of all active Constraint objects.
        """
        return {
            con
            for con in self._all
            if con.active and (category is None or con.category == category)
        }

    @property
    def detector(self) -> Dict[str, Optional[Union[float, Literal["True"]]]]:
        """Get active detector constraints.

        Returns
        -------
        Dict[str, Optional[Union[float, Literal["True"]]]]
            Dictionary with active detector constraint names and values.
        """
        return {con.name: con.value for con in self.constrained(CATEGORY.DETECTOR)}

    @property
    def reference(self) -> Dict[str, Optional[Union[float, Literal["True"]]]]:
        """Get active reference constraints.

        Returns
        -------
        Dict[str, Optional[Union[float, Literal["True"]]]]
            Dictionary with active reference constraint names and values.
        """
        return {con.name: con.value for con in self.constrained(CATEGORY.REFERENCE)}

    @property
    def sample(self) -> Dict[str, Optional[Union[float, Literal["True"]]]]:
        """Get active sample constraints.

        Returns
        -------
        Dict[str, Optional[Union[float, Literal["True"]]]]
            Dictionary with active sample constraint names and values.
        """
        return {con.name: con.value for con in self.constrained(CATEGORY.SAMPLE)}
    ...
    def all(
        ...
        """
        return {
            con.name: getattr(self, con.name)
            for con in self._all
            if category is None or con.category == category
        }
    ...
    @asdict.setter
    def asdict(self, constraints: Dict[str, Union[float, Literal["True"]]]) -> None:
        assert isinstance(constraints, dict)
        self.clear()
        for con_name, con_value in constraints.items():
            if hasattr(self, con_name) and isinstance(
                getattr(self, "_" + con_name), Constraint
            ):
    ...
    def astuple(self) -> Tuple[Union[Tuple[str, float], str], ...]:
        ...
        """
        res: List[Union[Tuple[str, float], str]] = []
        for cat in (CATEGORY.DETECTOR, CATEGORY.REFERENCE, CATEGORY.SAMPLE):
            for con in self.constrained(cat):
                if con.type is TYPE.VALUE:
                    res.append((con.name, getattr(self, con.name)))
                elif con.type is TYPE.VOID:
                    res.append(con.name)
                else:
                    raise DiffcalcException(
                        f"Invalid {con.name} constraint type found: {type(con.type)}"
                    )
        return tuple(res)
    ...
    def astuple(self, constraints: Tuple[Union[Tuple[str, float], str], ...]) -> None:
        ...
        for el in constraints:
            if (
                isinstance(el, str)
                and hasattr(self, el)
                and isinstance(getattr(self, "_" + el), Constraint)
            ):
                setattr(self, el, True)
            elif (
                isinstance(el, (type(("a", 1)), type(("a", 1.0))))
                and hasattr(self, el[0])
                and isinstance(getattr(self, "_" + el[0]), Constraint)
            ):
                setattr(self, *el)
            else:
    ...
    def is_fully_constrained(self, con: Optional[Constraint] = None) -> bool:
        ...
        count_constrained = len(self.constrained(con.category))
    ...
    def is_current_mode_implemented(self) -> bool:
        ...
        count_detector = len({c for c in self.detector})
        count_reference = len({c for c in self.reference})
        count_sample = len({c for c in self.sample})
        if count_sample == 3:
            if (
        ...
        if count_sample == 1:
            return self._omega not in set(
                self.constrained()
            ) and self._bisect not in set(self.constrained())
    ...
    def _get_factory(
        self, con: Constraint
    ) -> Callable[[], Optional[Union[float, Literal["True"]]]]:
        def _get_constraint() -> Optional[Union[float, Literal["True"]]]:
    ...
    def _set_factory(
        self, con: Constraint
    ) -> Callable[[Optional[Union[float, Literal["True"]]]], None]:
        def _set_value(val: Optional[Union[float, Literal["True"]]]) -> None:
            if con.type is TYPE.VOID:
                if val is True:
                    con.value = val
                    return
                    ...
                    raise DiffcalcException(
                        f"Constraint {con.name} requires boolean True value. "
                        f"Found {val} instead."
                    )
            elif con.type is TYPE.VALUE:
                try:
                ...
                raise DiffcalcException(
                    f"Invalid {con.name} constraint type found: {con.type}"
                )

        def _set_constraint(val: Optional[Union[float, Literal["True"]]]) -> None:
            if val is None or val is False:
                con.value = None
                return
            active_con = {c for c in self.constrained(con.category)}
    ...
    def _del_factory(self, con: Constraint) -> Callable[[], None]:
    ...
    @property
    def delta(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for delta angle."""
        return self._get_factory(self._delta)()

    @delta.setter
    def delta(self, val: Optional[float]) -> None:
    ...
    @property
    def nu(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for nu angle."""
        return self._get_factory(self._nu)()

    @nu.setter
    def nu(self, val: Optional[float]) -> None:
    ...
    @property
    def qaz(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for qaz angle."""
        return self._get_factory(self._qaz)()

    @qaz.setter
    def qaz(self, val: Optional[float]) -> None:
    ...
    @property
    def naz(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for naz angle."""
        return self._get_factory(self._naz)()

    @naz.setter
    def naz(self, val: Optional[float]) -> None:
    ...
    @property
    def beta(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for beta angle."""
        return self._get_factory(self._beta)()

    @beta.setter
    def beta(self, val: Optional[float]) -> None:
    ...
    @property
    def psi(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for psi angle."""
        return self._get_factory(self._psi)()

    @psi.setter
    def psi(self, val: Optional[float]) -> None:
    ...
    @property
    def eta(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for eta angle."""
        return self._get_factory(self._eta)()

    @eta.setter
    def eta(self, val: Optional[float]) -> None:
    ...
    @property
    def chi(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for chi angle."""
        return self._get_factory(self._chi)()

    @chi.setter
    def chi(self, val: Optional[float]) -> None:
    ...
    @property
    def phi(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for phi angle."""
        return self._get_factory(self._phi)()

    @phi.setter
    def phi(self, val: Optional[float]) -> None:
    ...
    @property
    def bisect(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for bisect mode."""
        return self._get_factory(self._bisect)()

    @bisect.setter
    def bisect(self, val: Optional[Literal["True"]]) -> None:
    ...
    @property
    def omega(self) -> Optional[Union[float, Literal["True"]]]:
        """Constraint for omega angle."""
        return self._get_factory(self._omega)()

    @omega.setter
    def omega(self, val: Optional[float]) -> None:
